...
- No WASM is compiled. `convert` runs `python -m mlc_llm convert_weight` + `gen_config` (the nightly wheel ships no `mlc_llm` console script) with `model_type`, `conv_template`, `context_window_size`, `prefill_chunk_size` copied from the `mlc-chat-config.json` of `mlc_reference`, then fails if any of `REFERENCE_KEYS` or `conv_template.name` differ. The prebuilt WASM only runs weights that match its compiled architecture.
